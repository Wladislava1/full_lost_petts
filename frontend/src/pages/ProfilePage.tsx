...
const UserAnnouncementsPage = () => {
  ...
  const currentUser = 'Владислава';
    const [userAnnouncements, setUserAnnouncements] = useState(
      dataTest.filter(item => item.ownerName === currentUser)
    );

  const userData = userAnnouncements.filter(item => item.ownerName === currentUser);
          ...
          <div className="relative inline-block">
            ...
            {showUserMenu && (
              <div className="absolute mt-2 right-0 md:left-0 bg-white rounded shadow-md w-48 z-50 overflow-hidden">
                <p className="px-4 py-2 hover:bg-gray-100 cursor-pointer">{currentUser}</p>
                <Link
                  to="/settings"
        ...
        <EditAnnouncementModal
          ...
          onSave={(updated) => {
            ...
            setUserAnnouncements(prev => 
              prev.map(a => (a === selectedAnnouncement ? updated : a))
            );
            setSelectedAnnouncement(null);
          }}
          onDelete={(deleted) => {
            if (confirm("Вы уверены, что хотите удалить это объявление?")) {
              setUserAnnouncements(prev => prev.filter(a => a !== deleted));
              setSelectedAnnouncement(null);
            }
          }}
        />
